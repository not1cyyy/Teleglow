...
rpl::producer<TextWithEntities> Text() {
	return tr::lng_about_text2(
		...
		rpl::single(Ui::Text::Link(
			"GNU GPL",
			"https://github.com/not1cyyy/Teleglow/blob/master/LICENSE")),
		lt_github_link,
		rpl::single(Ui::Text::Link(
			"GitHub",
			"https://github.com/not1cyyy/Teleglow")),
		tr::marked);
}
...
void AboutBox(not_null<Ui::GenericBox*> box, Window::SessionController* controller) {
	box->setTitle(rpl::single(u"Teleglow Desktop"_q));
	...
	box->addLeftButton(
		rpl::single(QString("@not1cyyy")),
		[box, controller]
		{
			box->closeBox();
			controller->showPeerByLink(Window::PeerByLinkInfo{
				.usernameOrId = QString("not1cyyy"),
			});
		});
