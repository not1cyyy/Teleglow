...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"Teleglow"_q : title);
...
, _log(this)
, _close(this) {
	_label.setText(u"Could not start Teleglow Desktop!\nYou can see complete log below:"_q);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time Teleglow Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time Teleglow Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST VERSION OF TELEGLOW DESKTOP"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://github.com/not1cyyy/Teleglow"_q);
	});
...
void LastCrashedWindow::saveReport() {
	QString to = QFileDialog::getSaveFileName(0, u"Teleglow Crash Report"_q, QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation) + u"/report.telegramcrash"_q, u"Telegram crash report (*.telegramcrash)"_q);
	if (!to.isEmpty()) {
		QFile file(to);
...
void LastCrashedWindow::checkingFinished() {
...

	{
		QString version = getReportField(qstr("version"), qstr("Version:"));
		if (!version.isEmpty()) {
			const auto sentryVersion = QString("teleglow-desktop@%1").arg(version);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding + QFontMetrics(_label.font()).horizontalAdvance(u"Last time Teleglow Desktop was not closed properly."_q) + padding + _networkSettings.width(), h);
	if (s == size()) {
		resizeEvent(0);
	} else {
		resize(s);
